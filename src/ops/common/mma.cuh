...
__device__ __forceinline__ void mma_fp8_e4m3(float& c0, float& c1, float& c2, float& c3,
    ...
    // The Blackwell-native form (kind::f8f6f4) is not accepted by ptxas for sm_89/sm_90. The classic
    // e4m3.e4m3.f32 form is accepted on every supported target (sm_89/90/100a/120a) and computes the
    // same operation, so non-Blackwell targets use it while Blackwell keeps its native encoding.
