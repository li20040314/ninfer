...
constexpr std::array<SupportSpec, 4> kSupports{{
    {5120, 6144, 6144},
    {5120, 17408, 17408},
    // Qwen3.5-9B text: attention/GDN output and MLP down projections (rows = hidden 4096).
    {4096, 4096, 4096},
    {4096, 12288, 12288},
}};
...
// 9B routes start from the nearest measured K (6144 for 4096, 17408 for 12288); no 4060
// performance data exists yet, so token-tile boundaries are expected to be retuned later.
...
constexpr std::array<RouteSpec, 6> kK12288Routes{{
    {{1, 1}, Q5LinearAddScheduleId::GemvResidual},
    {{2, 16}, Q5LinearAddScheduleId::Split2ExactResidual},
    {{17, 32}, Q5LinearAddScheduleId::MmaResidualR64C16},
    {{33, 48}, Q5LinearAddScheduleId::MmaResidualR64C24},
...
Q5LinearAddPlan q5_linear_add_resolve_plan(const Q5LinearAddProblem& problem) {
    ...
    case 6144:
        return resolve_from(kK6144Routes);
    case 12288:
        return resolve_from(kK12288Routes);
    case 17408:
        return resolve_from(kK17408Routes);
    }
    throw std::logic_error("q5 linear_add: supported shape has no route catalog");
}
